...
void transfer(mon* dest, mon* src, int index){
  ...
  /* } */
  /* puts(""); */
  long long int x = (src->items)[index];
  memmove(src->items+index, src->items+index+1, (src->num_items - index -1) * sizeof(long long int));
  src->num_items--;
...
void addx(long long int div, long long int* x){
  *x += div;
}
void mulx(long long int div, long long int* x){
  *x *= div;
}
void square(long long int div, long long int* x){
  /* printf("squaring %d\n", *x); */
  *x = *x * *x;
}
int test(long long int x, long long int div){
  return !(x%div);
}
void process_fgets(){
  ...
  int x;
  mon *monp;
  long long int *intarr = malloc(1000*sizeof(long long int));
  for(int i = 0; i < 10; ++i){
    (mons+i)->num_items = 0;
  ...
  while(round++ < 10000){
    for(int i = 0; i < 8; ++i){
      ...
      monp->count += (long long int) (monp->num_items);
      while (monp->num_items){
	(monp->op)(monp->num, monp->items);
	monp->items[0] = monp->items[0] % 9699690;
	/* printf("%d\n", monp->items[0]); */
	//monp->items[0] /= 3;
